...
protected:
	string color;
public:
	Cloud() : MoveableObject("Fluffy", 0, 0, 200, 5) { // random initial position on edge
		int edge = rand()%4;
		xpos = XSIZE/2 + cos(edge*M_PI/2)*XSIZE/2 + pow(-1,rand()%2)*sin(edge*M_PI/2)*XSIZE/2;
		ypos = YSIZE/2 + sin(edge*M_PI/2)*YSIZE/2 + pow(-1,rand()%2)*cos(edge*M_PI/2)*YSIZE/2;
		
		edge = (edge+rand()%3+1)%4; // different edge
		destinationX = XSIZE/2 + cos(edge*M_PI/2)*XSIZE/2 + pow(-1,rand()%2)*sin(edge*M_PI/2)*XSIZE/2;
		destinationX = YSIZE/2 + sin(edge*M_PI/2)*YSIZE/2 + pow(-1,rand()%2)*cos(edge*M_PI/2)*YSIZE/2;
	}
	...
	void draw() {
		...
		window.setpos(xpos+1.0/2, ypos+1.0/4*size);
		window.plot();
		...
		window.setpos(xpos +2.0/5*size, ypos+1.0/20*size);
		window.plot();
	}
	...
	void updatePosition()
	{
		...
		if(xpos == destinationX || ypos == destinationY)
		{
			*this = Cloud();
		}
	}
